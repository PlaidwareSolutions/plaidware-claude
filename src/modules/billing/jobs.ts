...
import { BILLING_SCHEDULE } from "./schedule";
...
export async function registerBillingJobs(boss: PgBoss): Promise<string[]> {
  ...
  await boss.schedule("billing.dunning-sweep", BILLING_SCHEDULE.dunningSweep.cron); // 14:00 UTC daily
  ...
  await boss.schedule("billing.hosting-invoices", BILLING_SCHEDULE.hostingInvoices.cron); // 08:00 UTC on the 1st
